  ...
  useEffect(() => {
    const handlePaymentVerification = async () => {
      ...
      const alreadyProcessed = sessionStorage.getItem(
        `payment_processed_${paymentIntentId}`,
      );
      if (alreadyProcessed) {
        console.log(
          "Payment already processed successfully, showing success state...",
        );
        setStatus("success");
      ...
      if (redirectStatus === "failed") {
        console.warn(
          "Payment was not completed - user may have canceled or payment failed at gateway",
        );
        setStatus("error");
        setMessage(
          "Payment was not completed at the payment gateway. Please try again.",
        );
      ...
      try {
        ...
        const verifyResponse = await fetch(
          `/api/payment/verify?payment_intent=${paymentIntentId}`,
        );
        ...
        if (pendingOrderId) {
          ...
          const orderResponse = await fetch(
            `/api/payment/pending-order?orderId=${pendingOrderId}`,
          );
        ...
        if (!orderData || !userData) {
          ...
          throw new Error(
            "Order information not found. Payment was successful but order could not be created. Please contact support with payment intent: " +
              paymentIntentId,
          );
        }
        ...
        if (!userData.userId) {
          ...
          throw new Error(
            "User information is incomplete. Please contact support with payment intent: " +
              paymentIntentId,
          );
        }
        ...
        if (!orderData.cart || orderData.cart.length === 0) {
          ...
          throw new Error(
            "Cart information is missing. Please contact support with payment intent: " +
              paymentIntentId,
          );
        }
