    ...
    <div className="animate-in fade-in duration-700">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            {isGuest && (
              <button
                ...
                className="group flex items-center gap-3 bg-transparent border-2 border-[#3e3a36] dark:border-white text-[#3e3a36] dark:text-white px-10 py-5 rounded-full font-bold transition-all hover:bg-[#3e3a36] hover:text-white dark:hover:bg-white dark:hover:text-[#3e3a36]"
              >
                <LogIn size={20} />
                Login
              </button>
      ...
      <section className="mb-20">
        <div className="flex justify-between items-end mb-12">
          <div>
            <h3 className="text-3xl font-black italic dark:text-white">
              Today&apos;s Harvest
            </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {featuredProducts.map((item) => (
            ...
            >
              <div
                className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-8 group-hover:scale-110 transition-transform duration-500 ${getCategoryAccent(item.category.name)}`}
              >
                {item.photo ? (
                  <Image
                    src={item.photo}
