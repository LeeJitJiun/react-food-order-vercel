...
interface ClientAppProps {
    ...
    orders: Order[];
    user: User;
    initialView?: 'home' | 'shop' | 'history' | 'profile' | 'login' | 'orderDetails';
...
export default function ClientApp({ products, categories, orders, user, initialView, selectedOrderId }: ClientAppProps) {
    const router = useRouter();
    const [isAuthenticated, setIsAuthenticated] = useState(false);
    const [currentUser, setCurrentUser] = useState<User>(user);
    const [view, setView] = useState<'home' | 'shop' | 'history' | 'profile' | 'login' | 'orderDetails'>(initialView || 'login');
    const [cart, setCart] = useState<CartItem[]>([]);
    const [isCheckoutOpen, setIsCheckoutOpen] = useState(false);
    const [status, setStatus] = useState<'idle' | 'ordering' | 'complete'>('idle');
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            // If initialView is provided, use it; otherwise default to 'home'
            if (initialView) {
                setView(initialView);
            } else {
                setView('home');
            ...
            setView('login');
            // Redirect to login page if not authenticated and currently on /home
    ...
    const handleLogout = () => {
        ...
        setCart([]);
        // Redirect to login page immediately without state changes
        window.location.href = '/login';
    };
    ...
    return (
        <div className="min-h-screen bg-[#f9f7f2] dark:bg-[#1a1816] text-[#3e3a36] dark:text-white/90 font-serif selection:bg-[#d4c8b8] flex transition-colors duration-500">
            ...
            <nav className="fixed left-6 top-1/2 -translate-y-1/2 z-40 bg-white/80 dark:bg-[#2d2a27]/80 backdrop-blur-xl border border-white dark:border-white/5 p-3 rounded-full shadow-2xl flex flex-col gap-4">
                <NavButton icon={Leaf} active={view === 'home' || view === 'shop'} onClick={() => router.push('/home')} />
                <NavButton icon={History} active={view === 'history' || view === 'orderDetails'} onClick={() => router.push('/history')} />
                <NavButton icon={User} active={view === 'profile'} onClick={() => router.push('/profile')} />
            ...
            <main className="flex-1 ml-28 p-10 max-w-7xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-10">
                    ...
                    {(view === 'shop' || view === 'history' || view === 'profile' || view === 'orderDetails') && (
                        <button
                            onClick={() => router.push(view === 'orderDetails' ? '/history' : '/home')}
                        ...
                        >
                            <ArrowLeft size={16} /> Back to {view === 'orderDetails' ? 'History' : 'Sanctuary'}
                        </button>
                    )}
                </div>
